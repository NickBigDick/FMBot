...
@bot.message_handler(commands=['find_family'])
def family_name(message: Message):
    if authorization(message):
        bot.send_message(message.chat.id, "Введите имя семейства")
        bot.set_state(message.from_user.id, UserState.find_family, message.chat.id)


@bot.message_handler(state=UserState.find_family)
def request(message: Message):
    user_id = get_user_fmid(message.from_user.username)
    if user_id:
        family_url = Family(family_name=message.text).url
        if family_url:
